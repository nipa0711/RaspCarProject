...
Mat SpeedSign(Mat res, Mat yuv)
{
	try
	{
		...
		Mat temp3[3];
		Mat ROI_Black;
		ROI.copyTo(ROI_Black);

		cvtColor(ROI_Black, ROI_Black, CV_RGB2HLS);
		...
		//imshow("num", cropedImage);

		drawContours(cropedImage, contours, largestIndex, color, CV_FILLED, 8);
		...
		if ((hu0 >= 600 && hu0 < 650) && (hu1 >= 150 && hu1 < 200) && (hu2>20 && hu2 < 30))
		{
			curOrder[5] = 0;
			curOrder[6] = 0;
			...
			if (curStat != limit30 && ROI.cols < 150)
			{
				...
				printf("Speed limit 30km/h\n");
				//printf("표지판 30이 검출되었습니다.\n");
				curStat = limit30;
			}
			if (ROI.cols > 50 && curOrder[4] != limit30)
			{
				printf("Mindstorm will ride 30km/h.\n");
				curOrder[4] = limit30;
			}
		}
		else if ((hu0 >= 200 && hu0 < 250) && (hu1 >= 5 && hu1 < 9))
		{
			...
			{
				frameCount = framePerSec * showSec;
				printf("Speed limit 40km/h\n");
				//printf("표지판 40이 검출되었습니다.\n");
				curStat = limit40;
			}
			if (ROI.cols > 50 && curOrder[5] != limit40)
			{
				printf("Mindstorm will ride 40km/h.\n");
				curOrder[5] = limit40;
			}
		}
		else if ((hu0 >= 450 && hu0 < 590) && (hu1 >= 60 && hu1<79) && (hu2>1.5 && hu2 < 8))
		{
			...
			{
				frameCount = framePerSec * showSec;
				printf("Speed limit 50km/h\n");
				//printf("표지판 50이 검출되었습니다.\n");
				curStat = limit50;
			}
			if (ROI.cols > 50 && curOrder[6] != limit50)
			{
				printf("Mindstorm will ride 50km/h.\n");
				curOrder[6] = limit50;
			}
		}
